...
def make_delayed_env(args, random_seed, delay_step):
    import gym
    # openai gym
    env_name = args.env_name

    env = gym.make(env_name)
    delayed_env = DelayedEnv(env, seed=random_seed, delay_step=delay_step)

    eval_env = gym.make(env_name)
    eval_delayed_env = DelayedEnv(eval_env, seed=random_seed, delay_step=delay_step)
...
def log_to_txt(args, total_step, result):
    ...
    delay = '(' + str(args.delay_step) + ')'
    f = open('./log/' + env_name + '_delay' + delay + '_seed' + seed + '.txt', 'a')
