...
class Twin_Q_net(nn.Module):
    def __init__(self, delay, state_dim, action_dim, device, hidden_dims=(256, 256), activation_fc=F.relu):
        super(Twin_Q_net, self).__init__()
        self.device = device

        self.activation_fc = activation_fc

        self.input_layer_A = nn.Linear(state_dim + (action_dim * delay) + action_dim, hidden_dims[0])
        ...
        self.input_layer_B = nn.Linear(state_dim + (action_dim * delay) + action_dim, hidden_dims[0])
...
class GaussianPolicy(nn.Module):
    def __init__(self, args, delay_step, state_dim, action_dim, action_bound,
        ...
        self.input_layer = nn.Linear(state_dim + delay_step * action_dim, hidden_dims[0])
