...
class ReplayMemory:
    def __init__(self, delay_step, state_dim, action_dim, device, capacity=1e6):
        self.device = device
        self.capacity = int(capacity)
        self.size = 0
        self.position = 0

        self.state_buffer = np.empty(shape=(self.capacity, state_dim + action_dim * delay_step), dtype=np.float32)
        ...
        self.next_state_buffer = np.empty(shape=(self.capacity, state_dim + action_dim * delay_step), dtype=np.float32)
