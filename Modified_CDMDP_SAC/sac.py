...
class SAC:
    def __init__(self, args, state_dim, action_dim, action_bound, action_space, device):
        ...
        self.device = device
        self.buffer = ReplayMemory(args.delay_step, state_dim, action_dim, device, args.buffer_size)
        self.batch_size = args.batch_size

        self.gamma = args.gamma
        self.tau = args.tau

        self.actor = GaussianPolicy(args, args.delay_step, state_dim, action_dim, action_bound, args.hidden_dims, F.relu, device).to(device)
        self.target_actor = GaussianPolicy(args, args.delay_step, state_dim, action_dim, action_bound, args.hidden_dims, F.relu, device).to(device)
        self.critic = Twin_Q_net(args.delay_step, state_dim, action_dim, device, args.hidden_dims).to(device)
        self.target_critic = Twin_Q_net(args.delay_step, state_dim, action_dim, device, args.hidden_dims).to(device)
